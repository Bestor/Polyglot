...
// embeds PocketBase for storage, syncs and caches match data from the
// configured Valorant data source, and exposes the small ANSI-SQL-oriented
// REST API described by openapi/polyglot.yaml (GET /query, POST /warm, GET
// /metadata) that any caller - including an MCP server - can use to
...
func main() {
	...
	app.OnServe().BindFunc(func(se *core.ServeEvent) error {
		...
		// which happens before this hook fires, so it's safe to open the
		// read-only query connection and introspect the schema here.
		query, err := ai.NewReadOnlyExecutor(cfg.PBDataDir)
		if err != nil {
			return err
		}

		if err := polyglot.RegisterRoutes(se, ing, query, cfg.APIAuthToken); err != nil {
			return err
		}
