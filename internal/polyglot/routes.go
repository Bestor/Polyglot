...
// (named data-fill functions, run asynchronously) and GET /warm (poll a
// warm job's status), GET /metadata (schema + function discovery), and
// GET/POST /datasources (onboard and describe DataProvider instances). It
// has no AI/reasoning logic of its own - that lives in a separate MCP
// server that calls these endpoints. It also has no domain knowledge of
// its own - every table/function comes from a dataprovider.Provider
// registered with the Registry (see cmd/polyglot/main.go).
package polyglot
...
func RegisterRoutes(se *core.ServeEvent, reg *Registry, query ai.QueryFunc, authToken string) error {
	...
	group.POST("/warm", handleWarm(reg, jobs))
